...
func generateBaseIcon(size: Int) -> NSImage? {
    guard let data = svg.data(using: .utf8), let dolphinImage = NSImage(data: data) else {
        return nil
    }
    ...
    let totalRect = NSRect(x: 0, y: 0, width: size, height: size)
    let margin = CGFloat(size) * 0.06
    let corner = CGFloat(size) * 0.224
    let cardRect = totalRect.insetBy(dx: margin, dy: margin)

    // Base squircle background with clean gradient
    let path = NSBezierPath(roundedRect: cardRect, xRadius: corner, yRadius: corner)
    
    // Gradient: Crisp clean white to soft azure
    let gradient = NSGradient(
        starting: NSColor(calibratedRed: 0.99, green: 0.99, blue: 1.0, alpha: 1.0),
        ending: NSColor(calibratedRed: 0.91, green: 0.95, blue: 1.0, alpha: 1.0)
    )
    gradient?.draw(in: path, angle: -90)

    // Subtle border
    NSColor(calibratedRed: 0.80, green: 0.87, blue: 0.99, alpha: 1.0).setStroke()
    path.lineWidth = CGFloat(size) * 0.015
    path.stroke()

    // Draw dolphin in center
    let dolphinWidth = cardRect.width * 0.70
    let dolphinHeight = dolphinWidth * (23.0 / 27.0)
    let dolphinX = cardRect.midX - (dolphinWidth / 2.0)
    let dolphinY = cardRect.midY - (dolphinHeight / 2.0)
    let dolphinRect = NSRect(x: dolphinX, y: dolphinY, width: dolphinWidth, height: dolphinHeight)

    dolphinImage.draw(in: dolphinRect, from: .zero, operation: .sourceOver, fraction: 1.0)
...
func savePNG(image: NSImage, targetSize: Int, fileURL: URL) {
    let rep = NSBitmapImageRep(
        bitmapDataPlanes: nil,
    ...
    image.draw(in: NSRect(x: 0, y: 0, width: targetSize, height: targetSize), from: .zero, operation: .copy, fraction: 1.0)
    NSGraphicsContext.restoreGraphicsState()
...
let iconsetDir = URL(fileURLWithPath: fm.currentDirectoryPath).appendingPathComponent("AppIcon.iconset")
try? fm.createDirectory(at: iconsetDir, withIntermediateDirectories: true)

guard let base1024 = generateBaseIcon(size: 1024) else {
...
for (name, px) in sizes {
    ...
    savePNG(image: base1024, targetSize: px, fileURL: url)
}
