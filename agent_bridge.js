(function() {
    ...
    console.log("[Agent Bridge] Initializing Tool Call Engine v4.3.25 (Cross-Platform Edition)...");
...
【闭环规则】：每次只输出一个代码块等真实结果，不编造；收到结果再决策；做完直接总结。铁律五条：①write_file 内容没准备好就别发块，空块会被直接忽略（无回执）；②local_cmd 发前自查括号配对 ()[]{}，配不平桥接层不会执行；③连 9222/CDP 前先跑 Get-NetTCPConnection -LocalPort 9222，无监听直接报"端口已退役"，不硬连；④Exit:1 且报错含 not recognized / Unexpected token / Missing expression → 说明命令被提取错了，换写法重发，严禁原样重发；⑤命令源码禁连续三个反引号（渲染层提前闭合必碎成多块），围栏内容一律用 [char]96 拼接，写 Markdown 文件优先 write_file。
...
请确认收到，并等待用户指令。`;
    ...
    function executeCommand(command, controller, opts) {
        ...
        try { diagAttach({ phase: 'dispatch', v: '4.3.25', cmd: normCmd.slice(0, 300) }); } catch (_) {}
