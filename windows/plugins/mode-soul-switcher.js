/* DSX 插件: mode-soul-switcher v1.0.5
 * v1.0.5: Start-Job与裸Start-ThreadJob双腿自动覆盖（shim+InitializationScript默认值；显式传-InitializationScript才需手动）。
...
Start-Job + Start-ThreadJob BOTH auto-load host cmdlets (shim + InitializationScript default). Manual host-DLL import only with explicit -InitializationScript. Detached work: prefer local_cmd:async fence (host runspace, all cmdlets ok) or detached Start-Process logging to C:/Windows/TEMP/opencode/job-NAME.log with Get-Content -Tail polling.
